  ...
  Future<PaginationSuccess<QuizItemModel>> getQuizItems({
    ...
    if (level == null) {
      ...
      resp = await supabase.rpc(
        'random',
        params: {'qid_value': quizId, 'limit_value': 30},
      );
    } else {
      /// 난이도가 존재한다. 1 2 3
      resp = await supabase.rpc(
    ...
    final quizItemModels = resp
        .map((item) => QuizItemModel.fromJson(item))
        .toList();
    return PaginationSuccess(items: quizItemModels);
  }
}
